...
namespace Repository
{
    public class AssetsRepository : LayerAbstractBase<AssetsDto>
    {
        ...
        public override async Task<IResultBase> AddAsync(AssetsDto LogData)
        {
            using (_dbConnection)
            {
                var query = "INSERT INTO Assets ( NAME, BRAND, COUNT, ACTIVE ) VALUES( @NAME, @BRAND, @COUNT, @ACTIVE )";
        ...
        public override async Task<IResultBase> AddMultipleAsync(IEnumerable<AssetsDto> LogData)
        {
            using (_dbConnection)
            {
                var query = "INSERT INTO Assets ( NAME, BRAND, COUNT, ACTIVE) VALUES(@NAME, @BRAND, @COUNT, @ACTIVE )";
        ...
        public override async Task<IDataResultBase<IEnumerable<AssetsDto>>> GetListAsync()
        {
            using (_dbConnection)
            {
                var query = "SELECT ID, NAME, BRAND, COUNT, ACTIVE FROM Assets";
                var data = await SqlMapper.QueryAsync<AssetsDto>(_dbConnection, query, commandType: CommandType.Text);

                return new DataResultBase<IEnumerable<AssetsDto>>(data.ToList(), true);
            }
        }
        ...
        public override async Task<IDataResultBase<AssetsDto>> GetAsync(int Id)
        {
            using (_dbConnection)
            {
                var query = "SELECT * FROM Assets WHERE ID = @ID";
                ...
                DynamicParameters parameters = new DynamicParameters();
                parameters.Add("@Id", Id);
                var data = await SqlMapper.QueryAsync<AssetsDto>(_dbConnection, query, parameters, commandType: CommandType.Text);
                return new DataResultBase<AssetsDto>(data.FirstOrDefault(), true);
            }
        }
        ...
        public override async Task<IResultBase> UpdateAsync(AssetsDto LogData)
        {
            using (_dbConnection)
            {
                var query = "UPDATE Assets SET NAME = @NAME, BRAND = @BRAND, COUNT = @COUNT, ACTIVE = @ACTIVE WHERE ID = @ID";
        ...
        public async Task<(List<AssetsDto> databaseTable, List<AssetsDto> databaseTable2)> GelMultipleDataResultAsync()
        {
            using (var multi = await _dbConnection.QueryMultipleAsync("SELECT * FROM Assets; SELECT * FROM Assets"))
            {
                var databaseTable = multi.Read<AssetsDto>().ToList();
                var databaseTable2 = multi.Read<AssetsDto>().ToList();
                return (databaseTable, databaseTable2);
        ...
        public override async Task<IDataResultBase<AssetsDto>> GetAsync(AssetsDto entity)
        {
            using (_dbConnection)
            {
                var query = "SELECT * FROM Assets WHERE BRAND = @BRAND, COUNT = @COUNT, ACTIVE = @ACTIVE";
